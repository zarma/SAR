...
//	_act1 = _obj addAction["Control vehicle","z_scripts\z_control_vehicle.sqf", [_unit,"start"], 1, false, false];
// 
//////////////////////////////////////////////////////////////////
...
if (_act=="start") then {
  ...
  _stop = player addAction["Stopper le contrôle","z_scripts\z_controle_vehicle.sqf",[_unit,"stop"], 1, false, false];
  for "_it" from 1 to _nb do {
		...
		if ((_unit getVariable "status")=="controlstopped") exitwith {"contrôle interrompu..." call z_smsg;player removeAction _stop;};
		_obj setVariable ["status", "controlled", true];
  };
  if ((_unit getVariable "status")=="controlled") then
  {
