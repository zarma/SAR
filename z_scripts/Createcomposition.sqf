...
_path = "\CA\modules\DynO\data\scripts\compositions\";
if (count _this > 3) then { _path = _this select 3 };

//player groupchat format ["_script=%1 _azi=%2 _pos=%3 _path=%4",_script,_azi,_pos,_path];

_objs = [];
_lib = format ["%1%2.sqf",_path,_script];
//player sidechat format ["_lib=%1",_lib];
//_objs = call (compile (preprocessFileLineNumbers format ["\CA\modules\DynO\data\scripts\compositions\%1.sqf",_script]));
_objs = call (compile (preprocessFileLineNumbers _lib));
private ["_posX", "_posY"];
_posX = _pos select 0;
_posY = _pos select 1;
_newObjs = [];
