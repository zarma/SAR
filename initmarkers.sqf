...
_missions =[];
_missionsf = [["mk017124","mk036131","mk072100"]] call Z_fnc_randomArray;//faciles
_missionsm = [["mk098067","mk104127"]] call Z_fnc_randomArray; // moyennes
_missionsd = [["mk096018","mk115040"]] call Z_fnc_randomArray; //difficiles
...
//	case 0: {_missions = _missionsf+_missionsm+_missionsd;};
	case 1: {_missions = _missionsf;};
	case 2: {_missions = _missionsm;};
	case 3: {_missions = _missionsd;};
};
...
if (count missions_chargees < Z_MAX_MISSIONS) then
{
  ...
  {
    if (missions_faciles_chargees < 3) then 
    {
      missions_chargees = [missions_chargees, _missions1 select 0] call BIS_fnc_arrayPush;
  ...
  {
  missions_chargees = [missions_chargees, _missions select 0] call BIS_fnc_arrayPush;
  _missions = [_missions, 0]  call BIS_fnc_removeIndex;
